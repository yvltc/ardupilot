...
{  
    ...
    // snprintf(buffer, sizeof(buffer), "invG: %.6f %.6f %.6f", invG.a.x, invG.a.y, invG.a.z);
    // gcs().send_text(MAV_SEVERITY_INFO, "%s", buffer);  // Send the formatted message
    // snprintf(buffer, sizeof(buffer), "invG: %.6f %.6f %.6f", invG.b.x, invG.b.y, invG.b.z);
    // gcs().send_text(MAV_SEVERITY_INFO, "%s", buffer);  // Send the formatted message
    // snprintf(buffer, sizeof(buffer), "invG: %.6f %.6f %.6f", invG.c.x, invG.c.y, invG.c.z);
    // gcs().send_text(MAV_SEVERITY_INFO, "%s", buffer);  // Send the formatted message
