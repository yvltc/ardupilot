...
const AP_Param::GroupInfo AC_GndPosControl::var_info[] = {
    ...
    // @Range: 1.000 3.000
    // @User: Standard
    AP_SUBGROUPINFO(_p_pos_z, "_POSZ_", 2, AC_GndPosControl, AC_P_1D),
    ...
    // @Units: Hz
    // @User: Advanced
    AP_SUBGROUPINFO(_pid_vel_z, "_VELZ_", 3, AC_GndPosControl, AC_PID_Basic),
    ...
    AP_SUBGROUPINFO(_pid_accel_z, "_ACCZ_", 4, AC_GndPosControl, AC_PID),
    ...
    // @Range: 0.500 2.000
    // @User: Standard
    AP_SUBGROUPINFO(_p_pos_xy, "_POSXY_", 5, AC_GndPosControl, AC_P_2D),
    ...
    // @Increment: 0.01
    // @User: Advanced
    AP_SUBGROUPINFO(_pid_vel_xy, "_VELXY_", 6, AC_GndPosControl, AC_PID_2D),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("_ANGLE_MAX", 7, AC_GndPosControl, _lean_angle_max, 0.0f),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("_JERK_XY", 10, AC_GndPosControl, _shaping_jerk_xy, POSCONTROL_JERK_XY),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("_JERK_Z", 11, AC_GndPosControl, _shaping_jerk_z, POSCONTROL_JERK_Z),
...
// their values.
//
AC_GndPosControl::AC_GndPosControl(AP_AHRS_View& ahrs, const AP_InertialNav& inav,
...
void AC_GndPosControl::input_pos_xyz(const Vector3p& pos, float pos_offset_z, float pos_offset_z_buffer)
{
...
float AC_GndPosControl::pos_offset_z_scaler(float pos_offset_z, float pos_offset_z_buffer) const
{
...
void AC_GndPosControl::set_max_speed_accel_xy(float speed_cms, float accel_cmss)
{
...
void AC_GndPosControl::set_correction_speed_accel_xy(float speed_cms, float accel_cmss)
{
...
void AC_GndPosControl::init_xy_controller_stopping_point()
{
    init_xy_controller();
...
void AC_GndPosControl::relax_velocity_controller_xy()
{
...
void AC_GndPosControl::soften_for_landing_xy()
{
...
void AC_GndPosControl::init_xy_controller()
{
...
///     The jerk limit defines the acceleration error decay in the kinematic path as the system approaches constant acceleration.
///     The jerk limit also defines the time taken to achieve the maximum acceleration.
void AC_GndPosControl::input_accel_xy(const Vector3f& accel)
{
...
void AC_GndPosControl::input_vel_accel_xy(Vector2f& vel, const Vector2f& accel, bool limit_output)
{
    update_pos_vel_accel_xy(_pos_target.xy(), _vel_desired.xy(), _accel_desired.xy(), _dt, _limit_vector.xy(), _p_pos_xy.get_error(), _pid_vel_xy.get_error());
...
///     The parameter limit_output specifies if the velocity and acceleration limits are applied to the sum of commanded and correction values or just correction.
void AC_GndPosControl::input_pos_vel_accel_xy(Vector2p& pos, Vector2f& vel, const Vector2f& accel, bool limit_output)
{
    update_pos_vel_accel_xy(_pos_target.xy(), _vel_desired.xy(), _accel_desired.xy(), _dt, _limit_vector.xy(), _p_pos_xy.get_error(), _pid_vel_xy.get_error());
...
void AC_GndPosControl::stop_pos_xy_stabilisation()
{
    _pos_target.xy() = _inav.get_position_xy_cm().topostype();
}
...
void AC_GndPosControl::stop_vel_xy_stabilisation()
{
    _pos_target.xy() =  _inav.get_position_xy_cm().topostype();
...
bool AC_GndPosControl::is_active_xy() const
{
...
void AC_GndPosControl::update_xy_controller()
{
    // check for ekf xy position reset
    handle_ekf_xy_reset();
...
///     This can be done at any time as changes in these parameters are handled smoothly
///     by the kinematic shaping.
void AC_GndPosControl::set_max_speed_accel_z(float speed_down, float speed_up, float accel_cmss)
{
...
///     speed_down can be positive or negative but will always be interpreted as a descent speed.
///     This should be done only during initialisation to avoid discontinuities
void AC_GndPosControl::set_correction_speed_accel_z(float speed_down, float speed_up, float accel_cmss)
{
...
void AC_GndPosControl::init_z_controller_no_descent()
{
...
///     This function should be used when the expected kinematic path assumes a stationary initial condition but does not specify a specific starting position.
///     The starting position can be retrieved by getting the position target using get_pos_target_cm() after calling this function.
void AC_GndPosControl::init_z_controller_stopping_point()
{
    // Initialise the position controller to the current throttle, position, velocity and acceleration.
    init_z_controller();
...
///     This function decays the output acceleration by 95% every half second to achieve a smooth transition to zero requested acceleration.
void AC_GndPosControl::relax_z_controller(float throttle_setting)
{
...
void AC_GndPosControl::init_z_controller()
{
...
///     The function takes the current position, velocity, and acceleration and calculates the required jerk limited adjustment to the acceleration for the next time dt.
void AC_GndPosControl::input_accel_z(float accel)
{
...
///     The parameter limit_output specifies if the velocity and acceleration limits are applied to the sum of commanded and correction values or just correction.
void AC_GndPosControl::input_vel_accel_z(float &vel, float accel, bool limit_output)
{
    // calculated increased maximum acceleration and jerk if over speed
    const float overspeed_gain = calculate_overspeed_gain();
    const float accel_max_z_cmss = _accel_max_z_cmss * overspeed_gain;
...
void AC_GndPosControl::set_pos_target_z_from_climb_rate_cm(float vel)
{
    // remove terrain offsets for flat earth assumption
    _pos_target.z -= _pos_offset_z;
    _vel_desired.z -= _vel_offset_z;
...
void AC_GndPosControl::land_at_climb_rate_cm(float vel, bool ignore_descent_limit)
{
...
///     The function alters the pos and vel to be the kinematic path based on accel
///     The parameter limit_output specifies if the velocity and acceleration limits are applied to the sum of commanded and correction values or just correction.
void AC_GndPosControl::input_pos_vel_accel_z(float &pos, float &vel, float accel, bool limit_output)
{
    // calculated increased maximum acceleration and jerk if over speed
    const float overspeed_gain = calculate_overspeed_gain();
    const float accel_max_z_cmss = _accel_max_z_cmss * overspeed_gain;
...
///     using the default position control kinematic path.
void AC_GndPosControl::set_alt_target_with_slew(float pos)
{
    float zero = 0;
...
void AC_GndPosControl::update_pos_offset_z(float pos_offset_z)
{
...
bool AC_GndPosControl::is_active_z() const
{
...
///     Desired velocity and accelerations are added to these corrections as they are calculated
///     Kinematically consistent target position and desired velocity and accelerations should be provided before calling this function
void AC_GndPosControl::update_z_controller()
{
...
float AC_GndPosControl::get_lean_angle_max_cd() const
{
...
void AC_GndPosControl::set_pos_vel_accel(const Vector3p& pos, const Vector3f& vel, const Vector3f& accel)
{
    _pos_target = pos;
    _vel_desired = vel;
...
/// set position, velocity and acceleration targets
void AC_GndPosControl::set_pos_vel_accel_xy(const Vector2p& pos, const Vector2f& vel, const Vector2f& accel)
{
    _pos_target.xy() = pos;
    _vel_desired.xy() = vel;
...
Vector3f AC_GndPosControl::lean_angles_to_accel(const Vector3f& att_target_euler) const
{
...
Vector3f AC_GndPosControl::get_thrust_vector() const
{
...
void AC_GndPosControl::get_stopping_point_xy_cm(Vector2p &stopping_point) const
{
...
void AC_GndPosControl::get_stopping_point_z_cm(postype_t &stopping_point) const
{
...
int32_t AC_GndPosControl::get_bearing_to_target_cd() const
{
...
float AC_GndPosControl::get_throttle_with_vibration_override()
{
...
void AC_GndPosControl::standby_xyz_reset()
{
...
void AC_GndPosControl::write_log()
{
    if (is_active_xy()) {
...
float AC_GndPosControl::crosstrack_error() const
{
...
void AC_GndPosControl::accel_to_lean_angles(float accel_x_cmss, float accel_y_cmss, float& roll_target, float& pitch_target) const
{
...
void AC_GndPosControl::lean_angles_to_accel_xy(float& accel_x_cmss, float& accel_y_cmss) const
{
    // rotate our roll, pitch angles into lat/lon frame
...
bool AC_GndPosControl::calculate_yaw_and_rate_yaw()
{
...
float AC_GndPosControl::calculate_overspeed_gain()
{
...
void AC_GndPosControl::init_ekf_xy_reset()
{
    Vector2f pos_shift;
...
void AC_GndPosControl::handle_ekf_xy_reset()
{
...
void AC_GndPosControl::init_ekf_z_reset()
{
    float alt_shift;
...
void AC_GndPosControl::handle_ekf_z_reset()
{
    // check for position shift
    float alt_shift;
...
bool AC_GndPosControl::pre_arm_checks(const char *param_prefix,
                                   char *failure_msg,
...
bool AC_GndPosControl::has_good_timing(void) const
{
